...
export async function GET(
  ...
  if (!id.trim()) return json({ok:false, error:"ID da atividade ausente."}, 400);
  ...
  try {
    const response = await fetch(url, {
      ...
      body:JSON.stringify({
        secret,
        action:"getPedagogicalOrientation",
    ...
    return json({ok:true, orientation:result.orientation ?? null});
  } catch (error) {
    return json({ok:false, error:error instanceof Error ? error.message : "Falha ao carregar a orientação."}, 502);
  }
}
